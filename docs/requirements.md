...
>
> 2026-05-15 补充：热点发现与基线新季追踪拆分节奏。`daily-discover` 每日只做全网热点发现与数据纳入；`baseline-track` 独立运行，支持一次性追平和后续按 TMDb 持续更新状态例行追踪，并输出独立 baseline 报告。
...
2. **按独立节奏检测 A 库（运营业务库）中已有 TV 剧集的新季更新**（电影完全跳过）。该链路由 `baseline-track` 执行，当前上层调度暂定每周一次；追踪优先复用 TMDb detail 缓存，并按 TMDb `status` / `in_production` 判断哪些剧仍需例行追踪。
...
- **新增功能 2：** A 库已有 TV 剧集的主动追踪（独立于每日热点发现，按 TMDb `status` / `in_production` 智能追踪，电影跳过）
...
7. **结果沉淀到中间表**（当前为本地 MD/JSON 报告，飞书同步为后续运营层需求） + 生成热点报告和基线报告
...
- ✅ A 库还作为功能 2 的追踪源（TV 剧集主动追踪新季，电影跳过）
...
- 连载剧新增单集。
- 新季发布。
- **基线内容主动追踪（V1.5 新增，仅 TV 剧集，电影跳过）。** 独立于每日热点发现，按 TMDb 剧集 `status` / `in_production` 字段智能追踪。
- 每日自动运行热点发现。
- 支持按独立节奏运行基线追踪（当前上层调度暂定每周一次）。
- 手动立即运行。
- 指定日期范围补采。
- 基于现有线上内容飞书表的冷启动。
...
| 命令 | 职责 | 频率 |
|------|------|------|
| `daily-discover` | 检测功能 1（全网热点），数据采集 + 评分 + 基线标记，**只写 B 库热点事件** | 每日 |
| `baseline-track` | 检测功能 2（A 库已有 TV 新季），支持 `routine` 例行追踪和 `catch-up` 一次性追平 | 当前暂定每周或人工触发 |
| `export-recommendations` | 从 B 库读取最近热点事件，导出 MD + JSON | 按需 |
| `export-baseline-updates` | 从 B 库读取最近基线新季事件，导出独立 MD + JSON | 按需 |

**调度位置：** 定时调度（cron、Airflow 等）在系统职责之外。系统只保证命令"随时可调"。这样 B 库具备"随时吐出建议"的能力，未来中间表替换为 Notion/Excel 时只需替换导出/同步层。
...
8. 接入前完成可访问性、解析稳定性、合规性的验证（详见 `reference/phase0_supplement.md`）。

### R13. 基线内容主动追踪（V1.5 核心新增需求，2026-05-15 调整节奏）

输入：

- A 库（飞书"线上内容基线表"）中的 TV 系列条目（电影完全跳过）。
- B 库 `virtual_series` 表（以 TMDb tv_id 聚合，详见 ADR-0007 § 决策三）。
- TMDb `tv/{id}` 接口或 24 小时内 detail cache（获取 `number_of_seasons`、`status`、`in_production`、episode air 信息、评分和热度字段）。
- 追踪调度配置（`catch-up` 全量追平；`routine` 按 TMDb 持续更新状态筛选）。

输出：

- 检测到的新季事件（写入 B 库 `content_updates` 表，`update_type=new_season`）。
- `is_in_baseline=true`（因为 series 在 A 库中存在）。
- `hot_score` 与评分拆解（复用现有热点评分公式，便于和热点报告横向比较）。
- `virtual_series.last_polled_at` 更新。
- `virtual_series.local_max_season` 同步到 TMDb 最新值。

验收标准：

1. `baseline-track --mode catch-up` 覆盖所有非 `skip` 的 A 库 TV 剧集，用于一次性追平。
2. `baseline-track --mode routine` 按 TMDb `status` / `in_production` 只追踪仍可能更新的剧集：
   - `Returning Series` / `In Production` → 纳入例行追踪
   - `Ended` / `Canceled` / `Pilot` / `Planned` → 追平后默认不做例行全量追踪
   - 空状态或未知状态 → 兜底纳入例行追踪
3. **电影完全跳过功能 2**（电影无"季"概念，续集发现由功能 1 通过热度榜兜底）。
4. 此功能**不受 `hot_score` 阈值过滤** —— A 库已有剧集的新季无论热不热都写入 B 库；但必须写入 `hot_score` 供运营排序参考。
5. 重复运行不重复写入相同 `content_update_id`。
6. 单条 TMDb API 失败不影响其他剧集继续轮询。
7. virtual_series 聚合失败的条目（TMDb tv_id 缺失或低置信度）跳过本次轮询，但记录到 `baseline_quality_issues` 表（承袭 ADR-0004 精神）。
8. 轮询日志记录每次的 API 调用次数、检测到的新季数、跳过原因。
9. baseline 报告独立导出，不混入每日热点报告。

**实现细节** 详见任务包 [`p1.5_c_virtual_series_backfill.md`](tasks/p1.5_c_virtual_series_backfill.md)、[`p1.5_d_baseline_active_tracking.md`](tasks/p1.5_d_baseline_active_tracking.md)、[`p1.18_split_baseline_tracking.md`](tasks/p1.18_split_baseline_tracking.md)。
