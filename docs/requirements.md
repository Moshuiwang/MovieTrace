...
更新事件主键用于标识一次具体业务更新，避免不同平台、地区或日期的上线事件被错误合并。

```text
update_event_id = canonical_item_id + platform + region + release_date + update_type
```

示例：

同一集在 Netflix 和 Prime Video 上线，应生成两条不同 update_event。  
同一集在不同上线日期或不同地区上线，也应生成不同 update_event。
...
| 内容类型 | 10 | 电视剧高于电影 |
| 平台权重 | 10 | Netflix、Prime Video 等重点平台更高 |
...
技术去重解决同一次或多次采集中重复采到同一个 update_event 的问题。
...
5. 同一剧集出现新季发布时，即使上一季已上架，也应作为新事件进入候选。
...
| platform | Netflix、Prime Video 等 |
| region | Global、US |
| release_date | 上线日期或预计上线日期 |
| language | 语言 |
| country | 制片国家/地区 |
| season_number | 季编号 |
| episode_number | 集编号 |
...
7. Netflix、Prime Video 等重点平台具有更高平台权重。
...
6. R6 不替代 R4。R4 是技术去重，R6 是业务流程去重。
