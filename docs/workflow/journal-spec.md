...
**跨日会话：** 文件名沿用会话开始的日期+时分；在日报内部"会话收尾"段标明跨日时间，如"2026-05-12 00:35 继续"。
...
1. **Agent 身份卡** — 工具名、模型、模型 ID、运行环境、起止 commit hash、**会话起止时间（精确到分钟，如 `2026-05-12 00:35 ~ 02:10`）**
2. **今日工作主线** — 每条主线包含：触发原因、结论、完成内容、关键发现；**关键事件标明发生时间（精确到分钟）**
