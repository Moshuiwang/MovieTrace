...
- [ ] **PR 前 Git 检查** — 按 [`.claude/rules/50-git-workflow.md`](../../.claude/rules/50-git-workflow.md) 确认分支基底、`STATE.md` 任务状态、`git diff --check`、验证命令、PR 描述和人类确认门禁
- [ ] **git commit / push / PR** — 上述变更按相关性统一提交；只有人类事前或当下明确确认后，才 push 并创建 PR。message 前缀使用：
  - `docs(meta):` — CLAUDE.md / AGENTS.md / STATE.md
  - `docs(state):` — 仅 STATE.md
  - `docs(decision):` — ADR 新增或状态变更
- [ ] **PR 后检查** — 仅在本会话已创建 PR 时执行：确认 PR checks、auto-merge、main CI/CD；合并后 `git checkout main && git pull --ff-only`
