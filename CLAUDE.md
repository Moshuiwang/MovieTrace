...
上下文压缩时优先保留：
- 当前 git 分支名（`git branch --show-current` 确认）
- 当前任务包 ID 和允许修改的文件列表
- 本会话做出的关键决策（验收标准、字段类型选择、数据来源选择等）
- 失败现象和已排除的方向（避免重复踩坑）
