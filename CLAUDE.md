...
> 状态：[`STATE.md`](STATE.md) · 规划：[`PLAN.md`](PLAN.md) · 地图：[`docs/context_map.md`](docs/context_map.md)
> Codex / 其他工具入口：[`AGENTS.md`](AGENTS.md)（一行指针 → 本文件）

**Boot Order**：STATE → context_map → 当前任务包。历史层先 `rg` 再读片段，不整篇读。
...
```bash
git pull                                # 1. 同步主干
git checkout -b feat/<task-id>-<slug>   # 2. 开本地分支
# ... 开发、测试、验证 ...
git commit && git push                  # 3. 提交并推送（此时无 PR，安全）
# 汇报结果，等用户说"开 PR"
gh pr create --base main --title "..."  # 4. 用户确认后才执行（触发 CI → auto-merge → 上线）
git checkout main && git pull           # 5. 合并后同步本地 main
```

**关键约束：** PR 必须最后创建，不得提前开。有 open PR 时 push 会直接触发 CI → auto-merge → 上线。
...
| 任务可做性判断（越界风险）| [`.claude/rules/10-scope-guardrails.md`](.claude/rules/10-scope-guardrails.md) |
